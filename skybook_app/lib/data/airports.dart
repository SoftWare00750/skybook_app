...
/// Looks up the [Airport] record for an IATA code (case-insensitive),
/// or null if it isn't in the curated list. Used to resolve a city/country
/// name for a flight's arrival code, e.g. for the destination-photo lookup
/// in `DestinationImages`.
Airport? airportForIata(String iata) {
  final code = iata.toUpperCase();
  for (final airport in airports) {
    if (airport.iata == code) return airport;
  }
  return null;
}
